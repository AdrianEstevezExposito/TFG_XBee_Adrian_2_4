...
diccionario = {"->luzremota":"E13:P0","->botonremoto":"E13:D0","->luzlocal":"P0","->potenciometro":"E13:D2",
		 "->simulaboton":"E13:CB","->detectacambios":"E13:IC"}
acciones = diccionario.keys()
comandos = diccionario.values()

dicmodos = {"desactivar":"0","rssi":"1","pruebaboton":"1","analogInput":"2","digitalInput":"3","apagar":"4","encender":"5","":""}
accionesM = dicmodos.keys()
comandosM = dicmodos.values()
...
def hiloPrincipal( du ):
  ...
  while True:
    ...
    if sNorm=="BREVE":
      ...
      continue
    if sNorm=="DICCIONARIO":
      print "Posibles comandos:"
      for acciones, comandos in diccionario.items():
	print "{} --> {}".format( acciones,comandos )
      continue

    #No es un comando vemos el tipo de AT
    m = re.search(r"([^:]*)->(.*)", s)
    if m: #se ha utilizado la flecha
      try: 
	print "Pasando por el diccionario: >{}<".format(diccionario[s])
      except KeyError:
	print "--La clave no coincide--"
      else:
	print "Posibles modos:"
	for accionesM, comandosM in dicmodos.items():
	  print "{} --> {}".format( accionesM,comandosM )
	try:
	  sn = raw_input("Esperando modo>")
	except EOFError: #EOF
	  break
	try: 
	  print "Pasando por el diccionario de modos: >{}<".format(dicmodos[sn])
	except KeyError:
	  print "--La clave no coincide--"
	else:
	    print "Resultado: >{}< en modo >{}<".format(diccionario[s],dicmodos[sn])
	    FComando = diccionario[s] + dicmodos[sn]
	    print "Comando a enviar: >{}< ".format(FComando)
	    m = re.search(r"([^:]*):(.*)", FComando)
		...
		print "Error al enviar comandos remotos '{}'".format( comandos )
	      ...
	      try:
		#print "Enviando comandos locales: >{}<".format(FComando)
		du.comandosATlocal( FComando )
	      except:
		print "Error al enviar comandos locales '{}'".format( FComando )
    else: #no entra por el diccionario
      m = re.search(r"([^:]*):(.*)", s)
      if m: #se ha utilizado los :
	( remota, comandos ) = m.groups()
	if len( remota )>0: #se epecifico nombre o dirección
